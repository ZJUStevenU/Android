...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float minDegree =0;
        float secDegree = 0;
        if(hour>=3 && hour <=15)hourDegree = (hour- 3)*30 ;
        if(hour < 3)hourDegree = (hour + 9)*30 + 0.5f*minute;
        if(hour > 15)hourDegree = (hour- 15)*30 + 0.5f*minute;

        if(minute >= 15)minDegree = (minute -15)*6 + second/10f;
        if(minute < 15)minDegree = (minute + 45)*6 + second/10f;

        if(second >= 15)secDegree = (second -15)*6 ;
        if(second < 15)secDegree = (second + 45)*6 ;
        ...
        float MendX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(minDegree)));
        float MendY = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minDegree)));
        canvas.drawLine(centerX , centerY ,MendX , MendY , needlePaint);

        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        float SendX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(secDegree)));
        float SendY = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secDegree)));
        canvas.drawLine(centerX , centerY ,SendX , SendY , needlePaint);
    }
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            float textWidth = numberPaint.measureText(strs[i]);
            canvas.drawText(
                    strs[i],
                    (float) (centerX + (radius * 0.8f + textWidth / 2f) * Math.cos(Math.toRadians(hourDegree))-20),
                    (float) (centerY + (radius * 0.8f + textHeight / 2f) * Math.sin(Math.toRadians(hourDegree))+15),
                    numberPaint
            );
        }
    }
